...
```bash
step-functions-debug$ sam build
```
...
```bash
step-functions-debug$ sam logs -n StockCheckerFunction --stack-name step-functions-debug --tail
```
...
```bash
step-functions-debug$ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name step-functions-debug
```
